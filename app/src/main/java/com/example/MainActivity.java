...
public class MainActivity extends AbstractActivity implements OnClickListener,
	...
	public static final int INITAL_COIN = 4000;
	public static final int INITAL_POOLID = 1;
	...
	protected void init() {
		level = pre.getInt(KEY_LEVEL, INITIAL_LEVEL);
		coin = pre.getInt(KEY_COIN, INITAL_COIN);
		poolId = pre.getInt(KEY_POOLID, INITAL_POOLID);
		...
		if (sModel == null) {
			sModel = getInitialData();
		}
		listModel.addAll(Arrays.asList(gson.fromJson(sModel, Model[].class)));
        if (listModel.size() > 1) {
	...
	void resetAll() {
        ...
        editor.putInt(KEY_LEVEL, INITIAL_LEVEL);
        editor.putInt(KEY_COIN, INITAL_COIN);
        editor.putInt(KEY_POOLID, INITAL_POOLID);

		editor.remove(KEY_SOLUTION);
		editor.remove(KEY_SUGGEST);
		editor.remove(KEY_WORD);
		editor.remove(KEY_MODELS);
